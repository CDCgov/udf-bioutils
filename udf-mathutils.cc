...
DoubleVal qt(FunctionContext* context, const DoubleVal& confidence, const BigIntVal& sample_size, const BooleanVal& two_tailed ) {
	if ( confidence.is_null || two_tailed.is_null || sample_size.is_null ) { return DoubleVal::null(); }
	if ( confidence.val > 1 || confidence.val < 0 || sample_size.val < 2 ) { return DoubleVal::null(); }

  	int64_t Sn = sample_size.val;
	double T = 0;

	// Rounding errors occur past this point. Since the T distribution converges toward the normal distribution as N grows large,
	// we can switch to using the normal distribution without much of a difference in the statistic.
	if ( Sn > 1404454273 ) {
		normal dist(0.0,1.0);
		if( two_tailed.val ) {
...
// Confidence interval function. Normally we take alpha but I think backwards, so we take the confidence level instead.
IMPALA_UDF_EXPORT
DoubleVal ci_t(FunctionContext* context, const DoubleVal& confidence, const BigIntVal& sample_size, const DoubleVal& sample_std, const BooleanVal& two_tailed ) {
	if ( confidence.is_null || two_tailed.is_null || sample_size.is_null || sample_std.is_null ) { return DoubleVal::null(); }
	if ( confidence.val > 1 || confidence.val < 0 || sample_size.val < 2 || sample_std.val < 0 ) { return DoubleVal::null(); }
